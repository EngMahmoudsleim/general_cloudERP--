...
class TransactionUtil extends Util
{
     ...
     */
    public function getLedgerDetails0000000000($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
    ...
    // Get contact (نحتاج نوعه Customer / Supplier / Both)
    $contact = Contact::find($contact_id);

    /**
     * ==============================
     * 1) Previous Transactions (قبل start date)
     * ==============================
     */
    $previous_transaction_sums = $this->__transactionQuery($contact_id, $start, null, $location_id)
        ->select(
            ...
            DB::raw("SUM(IF(type = 'purchase_return', final_total, 0)) as total_purchase_return"),
            DB::raw("SUM(IF(type = 'opening_balance', final_total, 0)) as total_opening_balance"),
            // فصل خصم الدفتر بيع/شراء
            DB::raw("SUM(IF(type = 'ledger_discount' AND sub_type = 'sell_discount', final_total, 0)) as total_sell_ledger_discount"),
            DB::raw("SUM(IF(type = 'ledger_discount' AND sub_type = 'purchase_discount', final_total, 0)) as total_purchase_ledger_discount"),
            DB::raw("SUM(IF(type = 'ledger_discount', final_total, 0)) as total_ledger_discount")
        )->first();

    // Payments قبل start date
    $prev_payments = $this->__paymentQuery($contact_id, $start, null, $location_id)
        ->select('transaction_payments.*', 'bl.name as location_name', 't.type as transaction_type', 'is_advance')
        ->get();
    ...
    // تم ترك حساب advance هنا لو احتجته لاحقاً، لكن لن نضيفه في beginning_balance (كما في النسخة FIXED)
    $prev_total_advance_payment = $this->__paymentQuery($contact_id, $start, null, $location_id)
        ->select(
            'bl.name as location_name',
            't.type as transaction_type',
    ...
    /**
     * ==============================
     * 2) Beginning Balance حسب نوع الكونتكت (FIXED LOGIC)
     * ==============================
     */
    $total_prev_invoice = 0;
    $total_prev_paid    = 0;

    $total_sell_ledger_discount_prev     = $previous_transaction_sums->total_sell_ledger_discount;
    $total_purchase_ledger_discount_prev = $previous_transaction_sums->total_purchase_ledger_discount;

    if ($contact && $contact->type == 'customer') {
        $total_prev_invoice = $previous_transaction_sums->total_invoice
                           - $previous_transaction_sums->total_sell_return
                           + $previous_transaction_sums->total_opening_balance
                           - $total_sell_ledger_discount_prev;

        $total_prev_paid = $prev_total_invoice_paid
                         - $prev_total_sell_return_paid
                         + $prev_total_ob_paid;

    } elseif ($contact && $contact->type == 'supplier') {
        $total_prev_invoice = $previous_transaction_sums->total_purchase
                           - $previous_transaction_sums->total_purchase_return
                           + $previous_transaction_sums->total_opening_balance
                           - $total_purchase_ledger_discount_prev;

        $total_prev_paid = $prev_total_purchase_paid
                         - $prev_total_purchase_return_paid
                         + $prev_total_ob_paid;

    } else { // both or null
        $total_prev_invoice = $previous_transaction_sums->total_purchase
                           + $previous_transaction_sums->total_invoice
                           - $previous_transaction_sums->total_sell_return
                           - $previous_transaction_sums->total_purchase_return
                           + $previous_transaction_sums->total_opening_balance
                           - $total_sell_ledger_discount_prev
        ...
        $total_prev_paid = $prev_total_invoice_paid
                         + $prev_total_purchase_paid
                         - $prev_total_sell_return_paid
                         - $prev_total_purchase_return_paid
                         + $prev_total_ob_paid;
    }

    // NOTE: لا نضيف $prev_total_advance_payment هنا (كما في FIXED) لتفادي double-count
    $beginning_balance = $total_prev_invoice - $total_prev_paid;

    /**
     * ==============================
     * 3) إعداد الـ with للعلاقات (للـ formats 3/4)
     * ==============================
     */
    $with = ['location'];
    if ($line_details) {
        $with = [
            'location',
            'sell_lines', 'sell_lines.sub_unit', 'sell_lines.product',
            'sell_lines.variations', 'sell_lines.product.unit', 'sell_lines.variations.product_variation',
            'sell_lines.line_tax',
            'purchase_lines', 'purchase_lines.product', 'purchase_lines.variations',
            'purchase_lines.variations.product_variation', 'purchase_lines.line_tax',
            'purchase_lines.product.unit', 'purchase_lines.product.unit.sub_units',
        ];
    }
    ...
    /**
     * ==============================
     * 4) Transactions بين start و end
     * ==============================
     */
    $transaction_query = $this->__transactionQuery($contact_id, $start, $end, $location_id)
        ->with($with)
    ...
    // دعم format_2 و format_4 (كما في النسخة الأصلية)
    if ($format == 'format_2' || $format == 'format_4') {
        $transaction_query->leftjoin('transaction_payments as tp', 'tp.transaction_id', '=', 'transactions.id')
            ->addSelect(DB::raw('COALESCE(SUM(tp.amount), 0) as total_paid'))
            ->groupBy('transactions.id');
    ...
    // خصم الدفتر داخل الفترة
    $ledger_discount            = 0;
    $ledger_discount_sell       = 0;
    $ledger_discount_purchase   = 0;

    foreach ($transactions as $transaction) {
        if ($transaction->type == 'opening_balance') {
            // هيتضاف في الآخر كـ سطر Opening Balance
            $opening_balance += $transaction->final_total;
            continue;
        }
        ...
        if ($transaction->type == 'ledger_discount') {
            ...
            if ($transaction->sub_type == 'sell_discount') {
                $ledger_discount_sell += $transaction->final_total;
            } elseif ($transaction->sub_type == 'purchase_discount') {
                $ledger_discount_purchase += $transaction->final_total;
            }
        }
    ...
    foreach ($payments as $payment) {
        ...
        // تتبع reverse payments (من النسخة FIXED)
        if ($contact && $contact->type == 'customer'
        ...
        // إخفاء المدفوعات المعدّلة لأنها محسوبة بالفعل في advance
        if (!empty($payment->parent_id)) {
            continue;
        }
        ...
        $ledger[] = [
            ...
            'payment_method'    => !empty($paymentTypes[$payment->method]) ? $paymentTypes[$payment->method] : '',
            'payment_method_key'=> $payment->method,
            // نفس منطق النسخة الأصلية في الـ debit/credit مع الحفاظ على advance/payment_type
            'debit'             => in_array($payment->transaction_type, ['purchase', 'sell_return'])
                                    || ($payment->is_advance == 1 && $contact && $contact->type == 'supplier')
            ...
            'credit'            => (
                                        (in_array($payment->transaction_type, ['sell', 'purchase_return', 'opening_balance'])
                                            || ($payment->is_advance == 1 && $contact && in_array($contact->type, ['customer', 'both'])))
                                        && $payment->is_return == 0
                                    )
                                    || $payment->payment_type == 'credit'
                                    ? $payment->amount : '',
    ...
    $total_advance_payment = $this->__paymentQuery($contact_id, $start, $end, $location_id)
        ->select(DB::raw('SUM(transaction_payments.amount) as amount'))
        ->where('method', 'advance')
        ->get()
        ->sum('amount');
    ...
    // إجمالي المدفوع خلال الفترة (لا نغيّره للحفاظ على التوافق)
    $total_paid = $total_invoice_paid + $total_purchase_paid
                - $total_sell_return_paid - $total_purchase_return_paid
                + $total_excess_advance_payment - $total_advance_payment;

    $total_transactions_paid = $total_invoice_paid + $total_purchase_paid
                             - $total_sell_return_paid - $total_purchase_return_paid;

    /**
     * ==============================
     * 7) Curr Due (محسّن حسب نوع الكونتكت) – من منطق النسخة FIXED
     * ==============================
     */
    if ($contact && $contact->type == 'customer') {
        $curr_due = $total_invoice
                  - $total_invoice_paid
                  + $total_sell_return_paid
                  + $beginning_balance
                  + $opening_balance_due
                  - $ledger_discount_sell
                  - $total_reverse_payment;
    } elseif ($contact && $contact->type == 'supplier') {
        $curr_due = $total_purchase
                  - $total_purchase_paid
                  + $total_purchase_return_paid
                  + $beginning_balance
                  + $opening_balance_due
                  - $ledger_discount_purchase
                  - $total_reverse_payment;
    } else { // both
        $curr_due = ($total_invoice + $total_purchase)
                  - ($total_invoice_paid + $total_purchase_paid)
                  + ($total_sell_return_paid + $total_purchase_return_paid)
                  + $beginning_balance
                  + $opening_balance_due
                  - ($ledger_discount_sell + $ledger_discount_purchase)
                  - $total_reverse_payment;
    }

    /**
     * ==============================
     * 8) ترتيب الـ ledger + إضافة Opening balance
     * ==============================
     */
    if (!empty($ledger)) {
        usort($ledger, function ($a, $b) {
    ...
    if ($format != 'format_2') {
        $ledger = array_merge([[
            ...
            'total'          => '',
            'payment_method' => '',
            'debit'          => $contact && $contact->type == 'customer' ? abs($total_opening_bal) : '',
            'credit'         => $contact && $contact->type == 'supplier' ? abs($total_opening_bal) : '',
            'others'         => '',
            'final_total'    => abs($total_opening_bal),
            'total_due'      => 0,
            'due_date'       => null,
    ...
    foreach ($ledger as $key => $val) {
        ...
        // Skip advance method (عشان ما نعدّهاش مرتين)
        if (!empty($val['payment_method_key']) && $val['payment_method_key'] == 'advance') {
            $credit = 0;
            $debit  = 0;
        }
    ...
    /**
     * ==============================
     * 10) Overall Totals (كل الفترات) – كما في النسخة الأصلية + الإبقاء على ledger_discount
     * ==============================
     */
    $overall_transaction_sums = $this->__transactionQuery($contact_id, null, null, $location_id)
        ->select(
    ...
    $total_overall_invoice  = $overall_transaction_sums->total_invoice
                            - $overall_transaction_sums->total_sell_return
                            + $overall_transaction_sums->total_opening_balance
                            - $overall_transaction_sums->total_ledger_discount;

    $total_overall_purchase = $overall_transaction_sums->total_purchase
                            - $overall_transaction_sums->total_purchase_return;
    ...
    $total_overall_paid_customer = $overall_total_invoice_paid
                                 - $overall_total_sell_return_paid
                                 + $overall_total_ob_paid; // ممكن تضيف + $overall_total_advance_payment لو حابب

    $total_overall_paid_supplier = $overall_total_purchase_paid
                                 - $overall_total_purchase_return_paid;

    $overall_due = $total_overall_invoice
                 + $total_overall_purchase
                 - $total_overall_paid_customer
                 - $total_overall_paid_supplier;
    ...
    $output = [
        ...
        'total_purchase'    => $total_purchase,
        'beginning_balance' => $beginning_balance + $opening_balance_due,
        'balance_due'       => $curr_due, // محسّن حسب نوع الكونتكت
        'total_paid'        => $total_paid,
        'total_reverse_payment' => $total_reverse_payment,

        // خصومات الدفتر داخل الفترة
        'ledger_discount'          => $ledger_discount,
        'ledger_discount_sell'     => $ledger_discount_sell,
        'ledger_discount_purchase' => $ledger_discount_purchase,

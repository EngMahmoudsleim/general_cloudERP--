...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        $installedModules = collect(Module::toCollection()->toArray())
            ->filter(fn ($module) => $moduleUtil->isModuleInstalled($module['name']))
            ->values();

        Menu::modify('admin-sidebar-menu', function ($menu) use ($installedModules, $moduleUtil) {
            $moduleAliases = $installedModules->pluck('alias')->filter()->values()->all();
            $menu->dropdown(
                __('lang_v1.modules'),
                function ($sub) use ($installedModules, $moduleUtil) {
                    foreach ($installedModules as $module) {
                        $moduleUtil->getModuleData('modifyAdminMenu', $sub, [$module['name']]);
                    }
                },
                [
                    'icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                    <path d="M4 8a2 2 0 0 1 2 -2h4a2 2 0 0 1 2 2v4a2 2 0 0 1 -2 2h-4a2 2 0 0 1 -2 -2z"></path>
                    <path d="M14 14a2 2 0 0 1 2 -2h4a2 2 0 0 1 2 2v4a2 2 0 0 1 -2 2h-4a2 2 0 0 1 -2 -2z"></path>
                    <path d="M15 6l2 -2"></path>
                    <path d="M18 6l-2 -2"></path>
                    <path d="M4 16l2 -2"></path>
                    <path d="M7 16l-2 -2"></path>
                  </svg>',
                    'active' => in_array(request()->segment(1), $moduleAliases),
                ]
            )->order(120);
        });

...
class ZatcaInvoiceController extends Controller
{
        ...
        try {
            ...
            foreach ($sell->sell_lines as $index => $line) {
                try {
                    ...
                    $product_name = $this->cleanItemName($product_name);
    ...
    /**
     * Clean item name by removing special characters using regex for ZATCA compliance
     * This function prevents INVOICE_HASHING_ERRORS by ensuring consistent XML generation
     * 
    ...
    private function cleanItemName($itemName)
    {
        ...
        // For ZATCA compliance, only allow alphanumeric characters and spaces
        // This prevents hashing errors caused by special characters in XML
        $cleanedName = preg_replace('/[^a-zA-Z0-9\s]/', '', $itemName);

    ...
    var global_sub_category_id = null;
    var global_is_clear_local_storage = false;
    var global_count_bugs = 0;
    
    /*#JCN 2025 to show the tooltip for the menu on the pos */
        $('button#btnCDisplay, button#btnCSuspend').hover(function () {
            $(this).tooltip('show');
        });
    
        $(document).on('mouseleave', 'button#btnCDisplay, button#btnCSuspend', function (e) {
            $(this).tooltip('hide');
        });
    /*END Tooltip*/
...
$(document).ready(function() {
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        // Check if quantity is 0 or negative and set it to 1
        if (entered_qty <= 0) {
            entered_qty = 1;
        ...
        adjustComboQty(tr);
            //#JCN POS Display V Own
            //innerHTML needs to be called before the element is appended to the DOM, wheras outerHTML needs to be called after it added to the DO
            //Using element.outerHTML to get the content (after)
            //Using localStorage.setItem to save data
            
            // tr.find('input[name^="quantity_cd"]').val(entered_qty.toFixed(2));
            // var dataHTML = tr.prop('outerHTML');
            //  console.log('XXXXXXXXXX pos_quantity XXXXXXXXXXXXX');
            //  console.log('outerHTML:' + dataHTML); 
            
            // //#JCN update data from the tr 
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price', function() {
        ...
        round_row_to_iraqi_dinnar(tr);
            ...
            //     var dataHTML = tr.prop('outerHTML');
            // console.log('XXXXXXXXXX pos_unit_price XXXXXXXXXXXXX');
            // console.log('outerHTML:' + dataHTML); 
            //     save_data_local_storage(dataHTML, 'update');
    });
    ...
    //# To remove/delete row on click in POS DISPLAY
    $('table#pos_table tbody').on('click', 'i.pos_remove_row', function() {
        var row = $(this).closest('tr');
        //#JCN POS Display V Own
        // var dataHTML = row.prop('outerHTML');
        // save_data_local_storage(dataHTML, 'remove');
        row.remove();
        pos_total_row();
        // save_footer_data();
    });
    ...
    $('button.pos-express-finalize').click(function() {
        ...
        /* Trying to avoid duplicate sales when press Shift+E*/
        ...
        /*
        global_count_bugs ++;
        console.log('Entrada Shift+E: ' +  global_count_bugs ); 
        */
    ...
    $('button#posEditDiscountModalUpdate').click(function() {
        ...
        // save_footer_data(); //# Using update to upd the info for the footer discuount, points...
    });
        ...
        .validate({
            ...
            messages: {
                contact_id: {
                    ...
                    number: LANG.only_numbers
                },               
                email: {
                    remote: LANG.email_taken,
                },
                mobile: {
                    minlength: LANG.length_mobile, 
                    number: LANG.only_numbers
                },
            },
            submitHandler: function(form) {
    ...
    $('select#select_location_id').on('change', function(e) {
        ...
        var location_id = $('input#location_id').val();
            ...
            //toastr.info("Previous category : " + global_p_category_id + " Location: " + location_id);
    
            //Change style to current tab category
            if (global_p_category_id)
            {
                //#JCN Current category ON in the tab
                change_status_tab(global_p_category_id, 1);
            }
    ...
    //It doesnt work in v6.0 (2024) I use the function .change() or .trigger('change') to trigger the event change in select#product_category
    //#JCN Begin: Added to show categories list in the window above products (div)
    //So i need to call the funtion get_product_suggestion_list() from here
    $('button#btn_product_category').on('click', function(e) {
        //#JCN 2025 changing the class for the active button
        // $(this).css('background', 'cyan');
        //#JCN getElementsByClassName returns a collection so we need to reference the first object
        //In this case in [0] return the object class="btn btn-prni active-7 ..." 
        //var aColl = document.getElementsByClassName('active-7')[0].style["background-color"] = '#fcf3cf';
        
        //#JCN previous category OFF in the tab
        change_status_tab(global_p_category_id, 0);
        change_status_tab(global_sub_category_id, 0);
        //toastr.warning("btn category : " + global_p_category_id);
   
        //Current tab category
        global_p_category_id = $(this).val();
        //#JCN Current category ON in the tab
        change_status_tab(global_p_category_id, 1)

        $name = $(this).data('name');
        //#JCN Add in the version 6 to start from the page = 1
        ...
        //#JCN Begin
        //Put the name of the category the first time in the input.
        $('input#name_category').val($(this).attr('title'));
        //console.log('END button#product_category Cate Value: ' +  $(this).val() + ' Nombre: ' + $(this).attr('title') );
    });
    // #JCN End

    // on click sub category in category drawer
    $('.product_subcategory').on('click', function(e) {

        //In this div  the data.maincategory is the parent/maincategory      
        //classname = 'active-' + $(this).data('value');
        //var el = document.getElementsByClassName(classname);
        parent = $(this).data('parent');
        maincategory = $(this).data('maincategory');
        change_status_tab(maincategory, 1, parent);
        global_sub_category_id = maincategory;
                   
        //#JCN Previous category OFF in the tab
        change_status_tab(global_p_category_id, 0, parent);
        global_p_category_id = $(this).data('value');

        //#JCN Current category ON in the tab
        change_status_tab(global_p_category_id, 1, parent)

        //#JCN Add in the version 6 to start from the page = 1
        $('input#suggestion_page').val(1);
        //var page = $('input#suggestion_page').val();
        //console.log('Sub category Page: ' +  page  );       
        var location_id = $('input#location_id').val();
        if (location_id != '' || location_id != undefined) {
            get_product_suggestion_list(
                global_p_category_id,
    ...
    //#JCN Begin
    //Put the name of the category the first time in the input.
    /* Using data-value=$category['id'] data-name=$category['name'] */
    $('input#name_category').val($(this).data('name'));
        get_featured_products();
    ...
    $('.main-category').on('click', function(){
        ...
        $('input#suggestion_page').val(1);
        //var page = $('input#suggestion_page').val();
        //console.log('Main category Page: ' +  page  ); 
        ...
        //toastr.warning('Main category'); 
        //Added #accordion to get the parent because data-parent = "#accordion" and replace data-parent = 0
        if (parent == 0 || parent === '#accordion') {
            get_product_suggestion_list(
                global_p_category_id,
                global_brand_id,
                $('input#location_id').val(),
    ...
    $('.main-category-div').hover (function() {
        ...
        //$('.main-category-div').trigger('click');
    }, function() {
        $(this).find('.sub-category-dropdown').hide();
        //$('.main-category-div').trigger('click');
    });

    $('.product_subcategory').on('click', function() {
        $('.sub-category-dropdown').hide();
    });

    $('.main-category-div').on('click', function() {
        elemid = document.getElementById('icon_subcate');

        theme_tab_up = 'fa-chevron-circle-up';
        theme_tab_down = 'fa-chevron-circle-down';            
        elem = document.getElementsByClassName('fa-chevron-circle-down');

        if (elem[0]){
            elemid.classList.remove(theme_tab_down);
            elemid.classList.add(theme_tab_up);
        }else{
            elemid.classList.remove(theme_tab_up);
            elemid.classList.add(theme_tab_down);
        }
    ...
    $(window).resize(function() {
        ...
        // In the reset form and when the location changes its neccesary to reset page 

        /*#JCN 2025-11-18 Reload the products if thw window is resized, layout responsive*/
        get_product_suggestion_list(
                    global_p_category_id,
                    global_brand_id,
                    $('input#location_id').val(),
...
function get_featured_products() {
    ...
    if (location_id && $('#featured_products_box').length > 0) {
         //#JCN the url from PosCustom/Routes/web.php
        $.ajax({
            method: 'GET',
            url: '/poscustom/sells/pos/get-featured-products-custom/' + location_id,
            dataType: 'html',
            success: function(result) {
                if (result) {
...
//#JCN 2025 funtion to Change the color in the TAB quick access categories
function change_status_tab(in_category_id, flag, parent = null  )
    {
        //toastr.info("calling function change_stat... :" + in_category_id + " Flag: " + flag);
        classname = 'active-' + in_category_id ;
        color_system = $('input#color_category_tab').val(); //Color of the system cfg
        el = document.getElementsByClassName(classname);
        /*Use top set the theme gradient */
        /* In this method it allowed to add one by one string in the class */
        theme_tab_g = 'tw-bg-gradient-to-r';
        theme_tab_f = 'tw-from-'+color_system+'-800'
        theme_tab_t = 'tw-to-'+color_system+'-500';
        theme_tab1 = 'cardpanel';
                               
        if (el[0] ) //Use for the drawer categories 
        {
            //#Flag ON=1 OFF=0
            if (flag == 1){
                //Case category bar
                el[0].classList.add(theme_tab_g);
                el[0].style.color = 'white';
                el[0].style.border = "2px solid white";
            }else{
                //Case category bar
                el[0].classList.remove(theme_tab_g);
                el[0].style.backgroundColor = '#ffffff';
                el[0].style.color = 'black';
                el[0].style.border = "";
            }
        }
            //Exist 2 item in the array used for the quick menu categories
            if (el[1]){
                //#Flag ON=1 OFF=0
                if (flag == 1){
                    //Case category menu
                    el[1].classList.add(theme_tab_g);
                    el[1].classList.add(theme_tab_f);
                    el[1].classList.add(theme_tab_t);
                    el[1].classList.add(theme_tab1);
                    el[1].style.color = 'white';
                    el[1].style.border = "4px solid white";
                }else{
                    //Case category menu
                    el[1].classList.remove(theme_tab_g);
                    el[1].classList.add(theme_tab_f);
                    el[1].classList.add(theme_tab_t);                    
                    el[1].classList.remove(theme_tab1);
                    //el[1].style.backgroundColor = '#ffffff';
...
function get_product_suggestion_list(category_id, brand_id, location_id, url = null, is_enabled_stock = null, repair_model_id = null) {
    ...
    /** #JCN 2024 To put a category by default it must be a parameter in the future
     * #JCN 2025 getting default category from pos_settings
    **/
    //toastr.warning('category: ' + category_id);

    if (category_id == null) {
        var category_id = $('input#ps_default_category').val();
        //toastr.info('category: ' + category_id);
        global_p_category_id = category_id;
        //#JCN Current category ON in the tab
        change_status_tab(global_p_category_id, 1);                
        $('input#name_category').val('VARIOS');
    } 

    /* #JCN To get the window size and make responsive and push records to paginate*/
    /* tw-h-dvh-items1 = Show category BAR tw-h-dvh-items0 = Dont Show */
    /*    
        var element = document.querySelector('.tw-h-dvh-items1') if no exist the class return null
        console.log(element);

        Main div to display the products
        var elem_div_main = document.getElementById('product_list_body');
        console.log(elem_div_main);

        each prodcut has w=114, h=114 the logic is to determinate how much elements_products is possible to draw in the div
        var w_paginate = ( Math.round(w/114)* Math.round(h/114));
    */

    //#JCN 2025 the width_product is set in settings 
    const w_product = $('input#ps_width_product').val();

    /*#JCN build a scale for the product to determinate the pagination*/

    const elem_div_main = document.getElementById('product_list_body'); 

    const w = elem_div_main.clientWidth;
    const h = elem_div_main.clientHeight;

           
/*         console.log('****** elem_div_main');
        console.log('Ancho:' + elem_div_main.clientWidth);
        console.log('Alto:' + elem_div_main.clientHeight); 
        console.log('w_product:' + w_product);  */
    

    
    const w_paginate = ( Math.round(w/w_product)* Math.round(h/w_product)); //Determina # de elementos a mostrar en el div
    //console.log('w_paginate = ' + w_paginate); 
    //const w_paginate = ( Math.round((w*h)/(w_product*w_product)));       
     
    //toastr.info('WSize:[ W=' + w + '*H=' + h +' ] ('+w_product  + ') Paginate:[' + w_paginate + ']' );

    if (url == null) {
        //#JCN the url from PosCustom/Routes/web.php add w to send the window size
        url = '/poscustom/sells/pos/get-product-suggestion-custom/' + w_paginate;
    }
    
    //#JCN
    /*
    toastr.warning('usando pos_custom.js in public/js');
    toastr.info(url);
    */
    ...
    if (page == 1) {
        ...
        //#JCN disable the previous button to avoid negative page
    ...
    $.ajax({
        ...
        dataType: 'html',
        success: function(result) {
            //#JCN This line is to prevent erase the content before paint the products results
            $('div#product_list_body').html('');
            //END
...
function get_recent_transactions(status, element_obj) {
    ...
    $.ajax({
        method: 'GET',
        url: '/poscustom/sells/pos/get-recent-transactions-custom',
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1) {
        ...
        $('#pos_table tbody')
            ...
            .each(function() {
                ...
                //#JCN POS Display V Own
                    ...
                    // save_data_local_storage(new_row_html, 'update');
                }
        });
    }
    ...
    if (add_via_ajax) {
        ...
        $.ajax({
            ...
            async: false,
            //#JCN it doesnt work in Hostinger async: true, 
            data: {
...
function pos_total_row() {
    ...
    // store on any update POSDisplay UPOS
    saveFormDataToLocalStorage();
...
function calculate_balance_due() {
    ...
    // store payment details POSDisplay UPOS
    saveFormDataToLocalStorage();
}
...
function reset_pos_form(){
    ...
    //#JCN The pos_redirect_url is in layouts/partials/header-pos
    //Modify to work with the module PosCustom
    if($('form#edit_pos_sell_form').length > 0){
        setTimeout(function() {
    ...
    // Set global_is_clear_local_storage to true to clear local storage POSDisplay UPOS
...
/*     $('button.cash').prop('disabled', false);
    ...
    //#JCN to refesh the POS with the new stock...
    //Call get_product_suggestion_list with the current parameters and reset global_p_category_id
    //to start all in the new location
...
//#JCN Rename  product_list_body1 to prevent scroll and use paginate
$('div#product_list_body1').on('scroll', function() {
...
//#JCN Begin
//Add to show page by page using previos or next page for the products
$('#next').on('click',function () {
    //toastr.success("Searching more products...");
    ...
    var page = parseInt($('#suggestion_page').val());
        ...
        //console.log('$(#next).on(click Cate Value: ' +  global_p_category_id + ' Nombre: ' + $(this).attr('title') );

        get_product_suggestion_list(global_p_category_id, brand_id, location_id, null, is_enabled_stock, device_model_id);
});
...
function save_data_local_storage(dataHTML, action = 'add') {
    var row = $(dataHTML);
    ...
    console.log('22222222222222222222222 modifiers 22222222222222222222222XXX'); */
    // Working withmodifiers
    var modifiers = [];
    ...
    if (action === 'add') {
        ...
        for (var i = 0; i < posData.length; i++) {
            if (posData[i].id === rowId) {
                posData[i].name = itemName;
                posData[i].price = itemPrice;
                posData[i].discount = itemDiscount;
                posData[i].quantity = itemQuantity;
                posData[i].total = itemTotal;
                posData[i].modifiers = modifiers;
                posData[i].modifier_total = totalModifierAmount;
    ...
    //#JCN POS Display to work with the data in localStorage

...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	{{--#JCN 2025 Variables for POS-Display ---}}
	...
	<td style="width: auto;">
		...
		@endif
		@php
			//#JCN Validate if the product has variation is_dummy = 1 it doesnt have variation y a single product
			if ($product->is_dummy){
				$product_name = $product->product_name /* . '<br/>' . $product->sub_sku */ ; //#JCN Hide the SKU
			} else {
				# code...
				$product_name = $product->product_variation_name .'('. $product->variation_name .')' ;
			}
			//#JCN Adding brand
		...
		{{--#JCN to have the title and the image in one line--}}
		...
		{{-- 
				@if(!is_null($product->media) && count($product->media) > 0)

					<img src="{{$product->media->first()->display_url}}" alt="Image" width="50">
				@elseif(!empty($product->product_image))
					<img src="{{asset('/uploads/img/' . rawurlencode($product->product_image))}}" alt="Image" width="50">
				@else
					<img src="{{asset('/img/default.png')}}" alt="Image" width="50" style="margin-right: 8px;">				
				@endif
		--}}
		...
		<small class="text-muted p-1">
			...
			@elseif ($product->product_type = 'combo')
				<small class="text-muted p-1">Combo</small> 
			@endif
		</small>
	...
	<td  style="width: auto;" >
		...
		<div class="input-group input-number">
			...
			<input type="text" data-min="1" style="@if (isMobile()) width: 50px; @else width: auto; @endif"
		...
		{{-- #JCN enable/disable from settings/POS needs settings_pos.blade--}} 
		{{--$pos_settings['poscustom_units_show']--}}	
		@if (!empty($pos_settings['poscustom_units_show']))
	...
	@else
		...
		@endif
	@endif
	<td class="{{$hide_tax}}" style="width: auto">
		{{--#JCN POS Display--}}
	...
	<td class="text-center v-center" style="width: auto">

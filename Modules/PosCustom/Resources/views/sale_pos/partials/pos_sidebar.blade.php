...
@php
    ...
    switch ($ps_width_product) {
        case '100':
            ...
            break;
        case '110':
            $ps_width_product = 120;
            break;            
        case '120':
            $ps_width_product = 132;
            break;                       
        case '140':
            $ps_width_product = 138; //141
            break;             
        default:
            $ps_width_product = 112; //Default with 105px still testing the best way 
            break;
    }
   
    //Case quick access category on top
    $winproduct_size = 'tw-h-dvh-cate-on' ; // Default 1 Show Category BAR In the CSS  
    if (!empty($pos_settings['poscustom_bar_show']) && !isMobile() )
        if ($poscustom_style_totals == 's0') //Buttons bottom
            $winproduct_size = 'tw-h-dvh-cate-on' ; // 1 Show Quick Category  
        else // s1 Buttons only left side $poscustom_style_totals == 's1' buttons on left only
            $winproduct_size = 'tw-h-dvh-bt_left_right_cate_on' ; // 1 Show Quick Category  
    else
        if ($poscustom_style_totals == 's0') //Buttons bottom
            $winproduct_size = 'tw-h-dvh-cate-off'; // 0 Dont Show Quick Category
        else //s1 Buttons only left side
            $winproduct_size = 'tw-h-dvh-bt_left_right_cate_off' ; //0 Dont Show Quick Category
...
{{-- winproduct_size pos_sidebar:
{{ $pos_settings['poscustom_width_product'] }} --}}
    ...
    {{--#JCN Ini common layout possidebar rgight side  --}}
...
{{--     $winproduct_size: 
    ...
    <div class="row"> <!--Products window-->
        ...
        <div class="col-md-12">
            ...
            <div class="{{ $winproduct_size }}  {{--tw-flex tw-flex-wrap --}} border tw-justify-items-stretch {{-- tw-p-1 tw-p-2 tw-h-screen --}} tw-text-center tw-overflow-y-auto tw-overflow-hidden" id="product_list_body">
            </div>
        </div>

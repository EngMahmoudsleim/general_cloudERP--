<!--#JCN Set the default high for items shop-->
@php
	//If button actions are in the left 
	$size_shop = "tw-h-dvh-shop0"; //Default

	if ($poscustom_style_totals == 's0') //Buttons bottom
            $size_shop = "tw-h-dvh-shop0";
    else //s1 Buttons only left side
       $size_shop = "tw-h-dvh-shop0lr";

@endphp	
{{  $poscustom_style_totals  }}
...
<div class="row">
	...
	<div class="col-md-4">
		...
		<!--#JCN Set the high for items shop-->
		@php
			$size_shop = "tw-h-dvh-shop1";
			if ($poscustom_style_totals == 's1') //Buttons bottom
            	$size_shop = "tw-h-dvh-shop1lr";
		@endphp	
	@endif
...
{{--#JCN 2025 responsive, trying to get the best way to make POS responsive
	Work in the lefside items and totals --}}

{{-- @php
	$id_winproduct_size = 'tw-h-dvh-65' ; // Show Category BAR In the CSS mode edit
@endphp

@if(($request->segment(3) == 'edit'|| $request->segment(4) == 'edit'))
    @php
       $id_winproduct_size = 'tw-h-dvh-60' ; // Show Category BAR In the CSS mode edit
    @endphp

@endif  --}}

{{-- <div class="row "> It doesnt work with the scrollbar for overflow--}}
	<div class="  @if (isMobile()) tw-h-dvh-shop1 @else {{ $size_shop }} @endif {{-- pos_product_div1--}}  {{-- space Left --}}  tw-overflow-y-auto tw-overflow-hidden ">
		...
		<table class="table table-condensed table-bordered table-striped table-responsive" id="pos_table">
			...
			<tbody>
				@foreach($sell_details as $sell_line)
					...
					@include('PosCustom::sale_pos.product_row', 
						['product' => $sell_line, 

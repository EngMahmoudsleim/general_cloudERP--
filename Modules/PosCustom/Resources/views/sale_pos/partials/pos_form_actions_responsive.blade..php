...
<!--Used to generate the tailwind styles-->
<div class="hidden border-2 border-blue-700 border-black border-purple-700 border-green-700 border-red-700 border-yellow-700 border-orange-700 border-sky-700 "></div>
<div class="hidden text-orange-700 hover:text-white border border-orange-700 hover:bg-orange-500 focus:ring-orange-300 
...
@if ($poscustom_btnstyle == 's1')
    ...
    <div class="pos-form-actions tw-bg-white tw-fixed tw-right-0 tw-bottom-0 tw-flex tw-p-1 tw-w-full !tw-items-center tw-justify-between flex-col md:flex-row sm:flex-col lg:flex-row  
                tw-rounded-tr-xl tw-rounded-tl-xl tw-shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] tw-border-t-2 {{ $border_pos_class }} {{-- tw-rounded-tr-xl tw-rounded-tl-xl --}}">
        ...
        <div class="tw-flex tw-flex-row tw-w-full !tw-items-center tw-justify-between md:!tw-hidden !tw-gap-3">
            ...
            @if (!Gate::check('disable_pay_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))        
                <button type="button" id="pos-finalize" title="@lang('lang_v1.tooltip_checkout_multi_pay')" 
                    ...
                    font-medium	 rounded-lg text-sm px-5 py-1.5 text-center {{-- me-2 mb-2 --}} 
                    dark:border-green-500 dark:text-green-500 dark:hover:text-white dark:hover:bg-green-600 dark:focus:ring-green-800">
                    <p class="text-nowrap"> <i class="fas fa-money-check-alt" aria-hidden="true">1 @lang('lang_v1.checkout_multi_pay') </i></p>
                </button>
            ...
            @if (empty($edit))
                    <button type="button"					
                        class="
                            no-print @if (!empty($pos_settings['disable_suspend']))  @endif pos-express-finalize @if (!array_key_exists('card', $payment_types)) hide @endif @if ($is_mobile) col-xs-6 @endif
                            text-red-700 hover:text-white border border-red-700 hover:bg-red-500 focus:ring-4 focus:outline-none focus:ring-red-300 
                            font-medium  rounded-lg text-sm px-5 py-1.5 text-center {{-- me-2 mb-2 --}}  dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-800"
            ...
            @else
                    <button type="button"					
                        class="no-print @if (!empty($pos_settings['disable_suspend']))  @endif pos-express-finalize @if (!array_key_exists('card', $payment_types)) hide @endif @if ($is_mobile) col-xs-6 @endif
                            text-red-700 hover:text-white border border-red-700 hover:bg-red-500 focus:ring-4 focus:outline-none focus:ring-red-300 
                            font-medium  tw-hidden hiderounded-lg text-sm px-5 py-1.5 text-center {{-- me-2 mb-2--}}  dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-800"
        ...
        <div class=" tw-flex tw-flex-row tw-w-full {{-- w-5/6  --}} tw-gap-4 tw-justify-center tw-hidden md:tw-flex tw-overflow-x-auto">
            
            @if (!Gate::check('disable_pay_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))        
                <button type="button" id="pos-finalize" title="@lang('lang_v1.tooltip_checkout_multi_pay')" 
                    class="tw-w-full text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-500 focus:ring-4 focus:outline-none focus:ring-blue-300 
                    font-medium	 rounded-lg text-sm px-5 py-1.5 text-center tw-p-2{{-- me-2 mb-2--}}  
            ...
            @if (!Gate::check('disable_express_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                <button type="button" 
                    class="pos-express-finalize tw-w-full
                        @if (!$is_mobile)  @endif no-print @if ($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif 
                        text-green-700 hover:text-white border border-green-700 hover:bg-green-500 focus:ring-4 focus:outline-none focus:ring-green-300 
                        font-medium  rounded-lg text-sm px-5 py-1.5 text-center {{-- me-2 mb-2--}}  dark:border-green-500 dark:text-green-500 dark:hover:text-white dark:hover:bg-green-600 dark:focus:ring-green-800"
                        data-pay_method="cash" title="@lang('tooltip.express_checkout')">
                    <p class="text-nowrap"> <i class="fas fa-money-bill-alt" aria-hidden="true"> @lang('lang_v1.express_checkout_cash') </i></p>
                </button>
            @endif
            ...
            @if (!Gate::check('disable_draft') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                <button type="button" 
                    class="tw-w-full @if ($pos_settings['disable_draft'] != 0) hide @endif 
                        ...
                        font-medium  rounded-lg text-sm px-5 py-1.5 text-center {{-- me-2 mb-2--}}  dark:border-orange-500 dark:text-orange-500 dark:hover:text-white dark:hover:bg-orange-600 dark:focus:ring-orange-800"
            ...
            @if (!Gate::check('disable_quotation') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                <button type="button"
                    class="tw-w-full text-amber-700 hover:text-white border border-amber-700 
                    hover:bg-amber-500 focus:ring-4 focus:outline-none focus:ring-amber-300 
                    font-medium rounded-lg text-sm px-5 py-1.5 text-center {{-- me-2 mb-2--}}  dark:border-amber-500 dark:text-amber-500 dark:hover:text-white dark:hover:bg-amber-600 dark:focus:ring-amber-900 @if ($is_mobile) col-xs-6 @endif"
            ...
            @if (!Gate::check('disable_suspend_sale') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                @if (empty($pos_settings['disable_suspend']))
                    <button type="button"					
                        class="tw-w-full no-print pos-express-finalize
                            ...
                            font-medium  rounded-lg text-sm px-5 py-1.5 text-center {{-- me-2 mb-2--}}  dark:border-yellow-500 dark:text-yellow-500 dark:hover:text-white dark:hover:bg-yellow-600 dark:focus:ring-yellow-800"
            ...
            @if (!Gate::check('disable_credit_sale') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                @if (empty($pos_settings['disable_credit_sale_button']))
                    ...
                    <button type="button"					
                        class="tw-w-full no-print pos-express-finalize @if ($is_mobile) col-xs-6 @endif
                            ...
                            font-medium  rounded-lg text-sm px-5 py-1.5 text-center {{-- me-2  mb-2--}} dark:border-emerald-500 dark:text-emerald-500 dark:hover:text-white dark:hover:bg-emerald-600 dark:focus:ring-emerald-800"
            ...
            @if (!Gate::check('disable_card') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    <button type="button"					
                        class="tw-w-full no-print @if (!empty($pos_settings['disable_suspend']))  @endif pos-express-finalize @if (!array_key_exists('card', $payment_types)) hide @endif @if ($is_mobile) col-xs-6 @endif
                            text-sky-700 hover:text-white border border-sky-700 hover:bg-sky-500 focus:ring-4 focus:outline-none focus:ring-sky-300 
                            font-medium  rounded-lg text-sm px-5 py-1.5 text-center {{-- me-2 mb-2--}}  dark:border-sky-500 dark:text-sky-500 dark:hover:text-white dark:hover:bg-sky-600 dark:focus:ring-sky-800"
            ...
            @if (!isset($pos_settings['hide_recent_trans']) || $pos_settings['hide_recent_trans'] == 0)
                    <button type="button"					
                        class="tw-w-full text-white bg-gradient-to-r from-{{ $theme_color }}-500 via-{{ $theme_color }}-600 to-{{ $theme_color }}-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-{{ $theme_color }}-300 dark:focus:ring-{{ $theme_color }}-800 shadow-lg shadow-{{ $theme_color }}-500/50 dark:shadow-lg dark:shadow-{{ $theme_color }}-800/80 font-medium rounded-lg text-sm 
                                px-5 py-1.5 text-center {{-- tw-h-11 --}} {{-- me-2 mb-2 --}}"
...
@elseif ($poscustom_btnstyle == 's1t')
    ...
    <div class="pos-form-actions tw-bg-white tw-fixed tw-right-0 tw-bottom-0 tw-flex tw-p-1 tw-w-full !tw-items-center tw-justify-between flex-col md:flex-row sm:flex-col lg:flex-row  
        ...
        {{-- Small devices essentials buttons--}} 
        <div class="tw-flex tw-flex-row tw-w-full tw-justify-center md:!tw-hidden !tw-gap-3">
        
            @if (!Gate::check('disable_pay_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))        
                <button type="button" id="pos-finalize" title="@lang('lang_v1.tooltip_checkout_multi_pay')" 
                    class="
                    text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-500 focus:ring-4 focus:outline-none focus:ring-blue-300 
                    font-medium	{{ $tab_class }} {{-- rounded-lg --}} text-sm px-5 py-1.5 text-center {{-- me-2 mb-2 --}} 
                    dark:border-green-500 dark:text-green-500 dark:hover:text-white dark:hover:bg-green-600 dark:focus:ring-green-800">
                    <p class="text-nowrap"> <i class="fas fa-money-check-alt" aria-hidden="true"> @lang('lang_v1.checkout_multi_pay') </i></p>
                </button>
            @endif     
            ...
            @if (empty($edit))
                    <button type="button"					
                        class="
                            no-print @if (!empty($pos_settings['disable_suspend']))  @endif pos-express-finalize @if (!array_key_exists('card', $payment_types)) hide @endif @if ($is_mobile) col-xs-6 @endif
                            text-red-700 hover:text-white border border-red-700 hover:bg-red-500 focus:ring-4 focus:outline-none focus:ring-red-300 
                            font-medium {{ $tab_class }} {{-- rounded-lg --}} text-sm px-5 py-1.5 text-center {{-- me-2 mb-2 --}}  dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-800"
                            id="pos-cancel" title="@lang('sale.cancel')">
                        <p class="text-nowrap"> <i class="fas fa-window-close" aria-hidden="true"> @lang('sale.cancel') </i></p>
                    </button>
            @else
                    <button type="button"					
                        class="no-print @if (!empty($pos_settings['disable_suspend']))  @endif pos-express-finalize @if (!array_key_exists('card', $payment_types)) hide @endif @if ($is_mobile) col-xs-6 @endif
                            text-red-700 hover:text-white border border-red-700 hover:bg-red-500 focus:ring-4 focus:outline-none focus:ring-red-300 
                            font-medium {{ $tab_class }} tw-hidden hide{{-- rounded-lg --}} text-sm px-5 py-1.5 text-center {{-- me-2 mb-2--}}  dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-800"
                            id="pos-delete" @if (!empty($only_payment)) disabled @endif title="@lang('messages.delete')">
                        <p class="text-nowrap"> <i class="fas fa-trash-alt" aria-hidden="true"> @lang('messages.delete') </i></p>
                    </button>
            @endif
        ...
        <div class=" tw-flex tw-flex-row tw-w-full {{-- w-5/6 --}} tw-gap-4 tw-justify-center tw-hidden md:tw-flex tw-overflow-x-auto">
            ...
            @if (!Gate::check('disable_pay_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))        
                <button type="button" id="pos-finalize" title="@lang('lang_v1.tooltip_checkout_multi_pay')" 
                    class="tw-w-full text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-500 focus:ring-4 focus:outline-none focus:ring-blue-300 
                    font-medium	{{ $tab_class }} {{-- rounded-lg --}} text-sm px-5 py-1.5 text-center tw-p-2{{-- me-2 mb-2--}}  
                    dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:hover:bg-blue-600 dark:focus:ring-blue-800">
                    <p class="text-nowrap"> <i class="fas fa-money-check-alt" aria-hidden="true"> @lang('lang_v1.checkout_multi_pay') </i></p>
                </button>
            @endif     
            ...
            @if (!Gate::check('disable_express_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                <button type="button" 
                    class="tw-w-full pos-express-finalize 
                        @if (!$is_mobile)  @endif no-print @if ($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif 
                        text-green-700 hover:text-white border border-green-700 hover:bg-green-500 focus:ring-4 focus:outline-none focus:ring-green-300 
                        font-medium {{ $tab_class }} {{-- rounded-lg --}} text-sm px-5 py-1.5 text-center {{-- me-2 mb-2--}}  dark:border-green-500 dark:text-green-500 dark:hover:text-white dark:hover:bg-green-600 dark:focus:ring-green-800"
                        data-pay_method="cash" title="@lang('tooltip.express_checkout')">
                    <p class="text-nowrap"> <i class="fas fa-money-bill-alt" aria-hidden="true"> @lang('lang_v1.express_checkout_cash') </i></p>
                </button>
            @endif
            ...
            @if (!Gate::check('disable_draft') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                <button type="button" 
                    class="tw-w-full @if ($pos_settings['disable_draft'] != 0) hide @endif 
                        text-orange-700 hover:text-white border border-orange-700 hover:bg-orange-500 focus:ring-4 focus:outline-none focus:ring-orange-300 
                        font-medium {{ $tab_class }} {{-- rounded-lg --}} text-sm px-5 py-1.5 text-center {{-- me-2 mb-2--}}  dark:border-orange-500 dark:text-orange-500 dark:hover:text-white dark:hover:bg-orange-600 dark:focus:ring-orange-800"
                        id="pos-draft" @if (!empty($only_payment)) disabled @endif> 
                    <p class="text-nowrap"> <i class="fas fa-edit " aria-hidden="true"> @lang('sale.draft') </i></p>
                </button>
            @endif
            ...
            @if (!Gate::check('disable_quotation') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                <button type="button"
                    class="tw-w-full text-amber-700 hover:text-white border border-amber-700 
                    hover:bg-amber-500 focus:ring-4 focus:outline-none focus:ring-amber-300 
                    {{ $tab_class }} font-medium {{-- rounded-lg --}} text-sm px-5 py-1.5 text-center {{-- me-2 mb-2--}}  dark:border-amber-500 dark:text-amber-500 dark:hover:text-white dark:hover:bg-amber-600 dark:focus:ring-amber-900 @if ($is_mobile) col-xs-6 @endif"
                    id="pos-quotation" @if (!empty($only_payment)) disabled @endif>
                    <p class="text-nowrap"><i class="fas fa-edit"></i> @lang('lang_v1.quotation')</p>
                    </button>
            @endif	
            ...
            @if (!Gate::check('disable_suspend_sale') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                @if (empty($pos_settings['disable_suspend']))
                    <button type="button"					
                        class="tw-w-full no-print pos-express-finalize
                            text-yellow-700 hover:text-white border border-yellow-700 hover:bg-yellow-500 focus:ring-4 focus:outline-none focus:ring-yellow-300 
                            font-medium {{ $tab_class }} {{-- rounded-lg --}} text-sm px-5 py-1.5 text-center {{-- me-2 mb-2--}}  dark:border-yellow-500 dark:text-yellow-500 dark:hover:text-white dark:hover:bg-yellow-600 dark:focus:ring-yellow-800"
                                data-pay_method="suspend" title="@lang('lang_v1.tooltip_suspend')"
                            @if (!empty($only_payment)) disabled @endif>
                        <p class="text-nowrap"> <i class="fas fa-pause" aria-hidden="true"> @lang('lang_v1.suspend') </i></p>
                    </button>
            ...
            @if (!Gate::check('disable_credit_sale') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                @if (empty($pos_settings['disable_credit_sale_button']))
                    ...
                    <button type="button"					
                        class="tw-w-full no-print pos-express-finalize @if ($is_mobile) col-xs-6 @endif
                            text-emerald-700 hover:text-white border border-emerald-700 hover:bg-emerald-500 focus:ring-4 focus:outline-none focus:ring-emerald-300 
                            font-medium {{ $tab_class }} {{-- rounded-lg --}} text-sm px-5 py-1.5 text-center {{-- me-2  mb-2--}} dark:border-emerald-500 dark:text-emerald-500 dark:hover:text-white dark:hover:bg-emerald-600 dark:focus:ring-emerald-800"
                            data-pay_method="credit_sale" title="@lang('lang_v1.tooltip_credit_sale')"
                        @if (!empty($only_payment)) disabled @endif>
                        <p class="text-nowrap"> <i class="fas fa-check" aria-hidden="true"> @lang('lang_v1.credit_sale') </i></p>
                    </button>
            ...
            @if (!Gate::check('disable_card') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    <button type="button"					
                        class="tw-w-full no-print @if (!empty($pos_settings['disable_suspend']))  @endif pos-express-finalize @if (!array_key_exists('card', $payment_types)) hide @endif @if ($is_mobile) col-xs-6 @endif
                            text-sky-700 hover:text-white border border-sky-700 hover:bg-sky-500 focus:ring-4 focus:outline-none focus:ring-sky-300 
                            font-medium {{ $tab_class }} {{-- rounded-lg --}} text-sm px-5 py-1.5 text-center {{-- me-2 mb-2--}}  dark:border-sky-500 dark:text-sky-500 dark:hover:text-white dark:hover:bg-sky-600 dark:focus:ring-sky-800"
                            data-pay_method="card" title="@lang('lang_v1.tooltip_express_checkout_card')">
                        <p class="text-nowrap"> <i class="fas fa-credit-card" aria-hidden="true"> @lang('lang_v1.express_checkout_card') </i></p>
                    </button>
            @endif
            ...
            @if (!isset($pos_settings['hide_recent_trans']) || $pos_settings['hide_recent_trans'] == 0)
                    <button type="button"					
                        class="{{ $tab_class }} tw-w-full text-white bg-gradient-to-r from-{{ $theme_color }}-500 via-{{ $theme_color }}-600 to-{{ $theme_color }}-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-{{ $theme_color }}-300 dark:focus:ring-{{ $theme_color }}-800 shadow-lg shadow-{{ $theme_color }}-500/50 dark:shadow-lg dark:shadow-{{ $theme_color }}-800/80 font-medium {{-- rounded-lg --}} text-sm 
                                px-5 py-1.5 text-center {{-- tw-h-11 --}} {{-- me-2 mb-2 --}}"
                            data-toggle="modal" data-target="#recent_transactions_modal" id="recent-transactions" title="@lang('lang_v1.recent_transactions')">
                        <p class="text-nowrap"> <i class="fas fa-clock" aria-hidden="true"> @lang('lang_v1.recent_transactions') </i></p>
                    </button>
...
@elseif ($poscustom_btnstyle == 's2')
    ...
    <div class="pos-form-actions {{ $theme_pos_class }} tw-bg-white tw-fixed tw-right-0 tw-bottom-0 tw-flex tw-p-1 tw-w-full !tw-items-center tw-justify-between flex-col md:flex-row sm:flex-col lg:flex-row  
        ...
        <div class="tw-flex tw-flex-row tw-w-full tw-gap-1 !tw-items-center tw-justify-between md:!tw-hidden">
            ...
            @if (!Gate::check('disable_pay_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))        
                <button type="button" id="pos-finalize" title="@lang('lang_v1.tooltip_checkout_multi_pay')" 
                    ...
                    text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 shadow-lg shadow-blue-500/50 dark:shadow-lg dark:shadow-blue-800/80 font-medium rounded-lg text-sm px-5 py-1.5 text-center {{-- me-2 mb-2 --}}">
                    <p class="text-nowrap"> <i class="fas fa-money-check-alt" aria-hidden="true"> @lang('lang_v1.checkout_multi_pay') </i></p>
                </button>
            @endif     
            ...
            @if (!Gate::check('disable_express_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                <button type="button" 
                    class="pos-express-finalize 
                        @if (!$is_mobile)  @endif no-print @if ($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif 
                        border tw-border-white text-white bg-gradient-to-r from-green-500 via-green-600 to-green-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 shadow-lg shadow-green-500/50 dark:shadow-lg dark:shadow-green-800/80 font-medium rounded-lg text-sm px-5 py-1.5 text-center {{-- me-2 mb-2 --}}"
                        data-pay_method="cash" title="@lang('tooltip.express_checkout')">
                    <p class="text-nowrap"> <i class="fas fa-money-bill-alt" aria-hidden="true"> @lang('lang_v1.express_checkout_cash') </i></p>
                </button>
            @endif
        ...
        {{-- Large devices essentials buttons--}} 
        <div class=" tw-flex tw-flex-row tw-w-full {{-- w-5/6 --}} tw-gap-4 tw-justify-center tw-hidden md:tw-flex tw-overflow-x-auto
            {{-- tw-flex tw-flex-row tw-w-full w-3/4  tw-gap-1 !tw-items-center tw-justify-between md:!tw-hidden --}}">
            @if (!Gate::check('disable_pay_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))        
                <button type="button" id="pos-finalize" title="@lang('lang_v1.tooltip_checkout_multi_pay')" 
                    class="tw-w-full border tw-border-white text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 shadow-lg shadow-blue-500/50 dark:shadow-lg dark:shadow-blue-800/80 font-medium rounded-lg text-sm px-5 py-1.5 text-center {{-- me-2 mb-2 --}}">
                    <p class="text-nowrap"> <i class="fas fa-money-check-alt" aria-hidden="true"> @lang('lang_v1.checkout_multi_pay') </i></p>
                </button>
            @endif     
...
@elseif ($poscustom_btnstyle == 's2t')
    ...
    <div class="pos-form-actions {{ $theme_pos_class }} {{-- tw-bg-white --}} tw-fixed tw-right-0 tw-bottom-0 tw-flex tw-p-1 tw-w-full !tw-items-center tw-justify-between flex-col md:flex-row sm:flex-col lg:flex-row  
        ...
        <div class="tw-flex tw-flex-row tw-w-full tw-gap-1 !tw-items-center tw-justify-between md:!tw-hidden">
            ...
            @if (!Gate::check('disable_pay_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))        
                <button type="button" id="pos-finalize" title="@lang('lang_v1.tooltip_checkout_multi_pay')" 
                    class="border tw-border-white {{ $tab_class }} 				
                        text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 shadow-lg shadow-blue-500/50 dark:shadow-lg dark:shadow-blue-800/80 font-medium {{-- rounded-lg --}} text-sm px-5 py-1.5 text-center {{-- me-2 mb-2 --}}">
                    <p class="text-nowrap"> <i class="fas fa-money-check-alt" aria-hidden="true"> @lang('lang_v1.checkout_multi_pay') </i></p>
                </button>
            @endif     
            ...
            @if (!Gate::check('disable_express_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                <button type="button" 
                    class="pos-express-finalize 
                        @if (!$is_mobile)  @endif no-print @if ($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif 
                        border tw-border-white {{ $tab_class }} text-white bg-gradient-to-r from-green-500 via-green-600 to-green-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 shadow-lg shadow-green-500/50 dark:shadow-lg dark:shadow-green-800/80 font-medium {{-- rounded-lg --}} text-sm px-5 py-1.5 text-center {{-- me-2 mb-2 --}}"
                        data-pay_method="cash" title="@lang('tooltip.express_checkout')">
                    <p class="text-nowrap"> <i class="fas fa-money-bill-alt" aria-hidden="true"> @lang('lang_v1.express_checkout_cash') </i></p>
                </button>
            @endif
        ...
        <div class=" tw-flex tw-flex-row tw-w-full {{-- w-5/6 --}} tw-gap-4 tw-justify-center tw-hidden md:tw-flex tw-overflow-x-auto
            ...
            @if (!isset($pos_settings['hide_recent_trans']) || $pos_settings['hide_recent_trans'] == 0)
                <button type="button"					
                    class="{{ $tab_class }} tw-w-full border tw-border-white text-white bg-gradient-to-r from-{{ $theme_color }}-500 via-{{ $theme_color }}-600 to-{{ $theme_color }}-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-{{ $theme_color }}-300 dark:focus:ring-{{ $theme_color }}-800 shadow-lg shadow-{{ $theme_color }}-500/50 dark:shadow-lg dark:shadow-{{ $theme_color }}-800/80 font-medium  {{-- rounded-lg --}} text-sm 
                        px-5 py-1.5 text-center {{-- tw-h-11 --}}{{-- me-2 mb-2 --}}"
                        data-toggle="modal" data-target="#recent_transactions_modal" id="recent-transactions" title="@lang('lang_v1.recent_transactions')">
                    <p class="text-nowrap"> <i class="fas fa-clock" aria-hidden="true"> @lang('lang_v1.recent_transactions') </i></p>
                </button>
                ...
                <div class="tw-w-full md:tw-w-fit tw-flex tw-flex-col tw-items-end {{-- tw-gap-3 --}} tw-hidden md:tw-block">
                    @if (!isset($pos_settings['hide_recent_trans']) || $pos_settings['hide_recent_trans'] == 0)
                        <button type="button"
                            ...
                            tw-px-5 py-1.5 {{-- tw-h-11 --}} tw-cursor-pointer tw-text-xs md:tw-text-sm"

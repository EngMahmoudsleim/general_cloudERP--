...
@section('content')
    ...
    <section class="{{-- content --}} no-print">
        ...
        <div class="hidden grid grid-rows-[1fr,auto] md:grid-cols-[1fr,45%] lg:grid-cols-[1fr,45%]">
             </div>   
        {{--Esta parte debe por fuera del maindiv de lo contarrio daña la parte resposive buscadno donde debe quedar--}}
        <!-- Original for Form::open 'class' => 'grid md:grid-cols-[60%,1fr] lg:grid-cols-[60%,1fr] tw-gap-1', -->
        <!-- This part must be custom md:grid-cols-[60%,1fr] ', -->
...
{{ $poscustom_style_totals }} --}}
        ...
        <pos-items-totals class = "tw-bg-white grid-cols-1 grid   @if ($poscustom_posi_totals == 'right' ) md:col-start-2  {{-- @else grid-rows-[auto,1fr]  --}} @endif md:row-start-2 md:block tw-overflow-y-auto tw-overflow-hidden tw-rounded-lg" >
            ...
            @endif tw-p-1">
                ...
                <!-- Style with totals & button action on left side-->
                @include('PosCustom::sale_pos.partials.pos_form') 
            </pos-sale-items>
...
{{-- #JCN
	...
	js working in public/modules/poscustom/js
--}}
...
{{-- #JCN--}}
    ...
    @if (in_array('tables', $enabled_modules) ||
            ...
            {{--#JCN Include restaurant.js to save modifiers for POS Display--}}
            @include('PosCustom::layouts.partials.js_custom.restaurant_js_custom')
            {{--     
